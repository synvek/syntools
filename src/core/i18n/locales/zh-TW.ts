...
const zhTW = {
  ...
  tools: {
    'video-convert': {
      formats: '支援格式：MP4 / MOV / WebM / MKV / AVI / MPEG-TS（不支援時自動回退 ffmpeg.wasm）',
      target: '目標格式',
      quality: '畫質',
      qualities: {
        low: '低',
      ...
      available: '可用',
      unavailable: '不可用',
      probe: '時長 {{duration}} · {{size}} · 視訊 {{video}} / 音訊 {{audio}}',
      convert: '開始轉碼',
      converting: '轉碼中…',
      cancel: '取消',
      progress: '處理中 {{percent}}%',
      download: '下載 {{name}}',
      usedEngine: '使用引擎：{{engine}}',
      errors: {
        INVALID_TARGET: '目標格式不受支援',
        INVALID_QUALITY: '畫質參數無效',
        INVALID_TRIM: '裁剪區間無效（結束需大於開始）',
        PROBE_FAILED: '無法讀取媒體資訊',
        WEBCodecs_VIDEO_MISSING: '瀏覽器不支援 WebCodecs 視訊編解碼，且 ffmpeg.wasm 不可用',
        WEBCodecs_AUDIO_MISSING: '瀏覽器不支援 WebCodecs 音訊編解碼，且 ffmpeg.wasm 不可用',
        NO_ENCODABLE_CODEC: '找不到可用的編碼器',
        WEBCodecs_FAILED: 'WebCodecs 處理失敗',
        UNSUPPORTED: '目前環境不支援 ffmpeg.wasm',
        NEED_ISOLATION: '需要跨域隔離（COOP/COEP）才能啟用多執行緒 ffmpeg.wasm',
        LOAD_FAILED: 'ffmpeg.wasm 核心載入失敗',
        EXEC_FAILED: 'ffmpeg 執行失敗',
        OUTPUT_MISSING: '未產生輸出檔案',
        CONVERT_FAILED: '轉換失敗',
      },
    },
    'video-to-gif': {
      formats: '支援格式：MP4 / MOV / WebM / MKV / AVI / MPEG-TS',
      usedEngine: '抽幀引擎：{{engine}}',
      start: '起始 (s)',
      duration: '時長 (s)',
      fps: '影格率',
      width: '寬度',
    ...
    'audio-convert': {
      formats: '支援格式：WAV / MP3 / M4A / AAC / OGG / Opus / FLAC / WMA / AIFF',
      target: '目標格式',
      quality: '音質',
      qualities: {
        low: '低',
    ...
    markdown: {
      ...
      draftRestored: '已還原上次草稿',
      copied: '已複製到剪貼簿',
      fileTooLarge: '檔案過大（上限 2 MB）',
      fileError: '操作失敗，請重試',
      dropHint: '把 .md 檔案拖到這裡即可開啟',
      gfm: 'GFM（表格 / 刪除線 / 任務列表）',
      breaks: '換行即換行（breaks）',
      input: 'Markdown 編輯',
      ...
      preview: '預覽',
      shortcuts:
        '快速鍵：⌘/Ctrl+B 粗體 · ⌘/Ctrl+I 斜體 · ⌘/Ctrl+K 連結 · ⌘/Ctrl+E 行內程式碼 · ⌘/Ctrl+S 儲存',
      toolbar: {
        task: '待辦清單',
        aria: 'Markdown 編輯工具欄',
        bold: '加粗（**）',
    ...
    doodle: {
      ...
      moreColors: '更多顏色',
      recentColors: '最近使用',
      colorHint: '左鍵設為前景色（描邊），右鍵設為背景色（填滿）',
      zoom: '縮放',
      zoomIn: '放大',
      zoomOut: '縮小',
      zoomFit: '適應視窗',
      polygonHint: '依序點擊新增頂點，點回起點或按 Enter 閉合，Backspace 退點，Esc 取消',
      moveHint: '拖曳移動整幅畫面',
      pickerHint: '點擊畫布取色',
      size: '粗細',
      opacity: '不透明度',
      ...
      customColor: '自訂顏色',
      background: '畫布底色',
      bg: { transparent: '透明', solid: '純色', grid: '網格' },
      canvasSize: '畫布',
      sizeFit: '自適應',
      sizeCustom: '自訂',
      width: '寬',
      ...
      undo: '復原',
      redo: '重做',
      hint: '在畫布上拖曳繪製，支援滑鼠、觸控與手寫筆壓感；色板上按右鍵可設為背景色',
      hintKeys:
        'Ctrl/⌘+Z 復原，Ctrl/⌘+Shift+Z 重做，數字鍵 1~9 切換工具，I 吸管 / V 移動，Enter 閉合多邊形',
      draftRestored: '已還原上次的草稿',
      tool: {
        picker: '吸管',
        polygon: '多邊形',
        move: '移動',
