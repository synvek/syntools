...
import { extractPdfPages } from './core';
...
export default function PdfExtractPagesTool() {
  ...
  const run = async () => {
    ...
    setBusy(true);
    setError(null);
    const r = await extractPdfPages(file, selection);
    setBusy(false);
    if (!r.ok) {
      setError(t(`tools.pdf-extract-pages.errors.${r.error}`));
      return;
    }
    downloadBytes(r.value, 'extracted.pdf');
  };

  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-gray-500 dark:text-gray-400">{t('tools.pdf-extract-pages.hint')}</p>
      ...
      <PdfField label={t('tools.pdf-extract-pages.pages')}>
        <input className={pdfInputClass} value={selection} onChange={(e) => setSelection(e.target.value)} placeholder="1,3-5" />
      </PdfField>
      {error && <p className="text-sm text-red-600 dark:text-red-400">{error}</p>}
      <div className="flex flex-wrap gap-2">
        <PdfRunButton label={busy ? t('common.loading') : t('tools.pdf-extract-pages.run')} disabled={!file || busy} onClick={() => void run()} />
