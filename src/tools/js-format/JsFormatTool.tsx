...
import { processJs, type IndentSize, type JsAction } from './core';
...
export default function JsFormatTool() {
  ...
  const [action, setAction] = useState<JsAction>(init.a === 'compress' ? 'compress' : 'format');
  const [indent, setIndent] = useState<IndentSize>(init.n === 4 ? 4 : 2);

  const result = useMemo(() => {
    if (!input.trim()) return null;
    return processJs(input, action, indent);
  }, [input, action, indent]);
    ...
    <div className="flex flex-col gap-4">
      <OptionBar>
        <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
          ...
          <select
            value={action}
            onChange={(e) => setAction(e.target.value as JsAction)}
          ...
          >
            <option value="format">{t('tools.js.actions.format')}</option>
            <option value="compress">{t('tools.js.actions.compress')}</option>
          </select>
        </label>
        <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
          {t('tools.js.indent')}
          ...
          >
            <option value={2}>{t('tools.js.indent2')}</option>
            <option value={4}>{t('tools.js.indent4')}</option>
          </select>
        </label>
      ...
      <div className="flex flex-col gap-4 lg:flex-row">
        <IOTextArea
          label={t('tools.js.input')}
          value={input}
          onChange={setInput}
          placeholder={t('tools.js.placeholder')}
      ...
      {result && !result.ok && (
        <p role="alert" className="text-sm text-red-600 dark:text-red-400">
          {translateToolError('tools.js', result)}
        </p>
      )}
    </div>
  );
