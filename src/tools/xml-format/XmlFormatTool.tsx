...
import { processXml, type IndentSize, type XmlAction } from './core';
...
export default function XmlFormatTool() {
  ...
  const [action, setAction] = useState<XmlAction>(init.a === 'compress' ? 'compress' : 'format');
  const [indent, setIndent] = useState<IndentSize>(init.n === 4 ? 4 : 2);

  const result = useMemo(() => {
    if (!input.trim()) return null;
    return processXml(input, action, indent);
  }, [input, action, indent]);
    ...
    <div className="flex flex-col gap-4">
      <OptionBar>
        <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
          ...
          <select
            value={action}
            onChange={(e) => setAction(e.target.value as XmlAction)}
          ...
          >
            <option value="format">{t('tools.xml.actions.format')}</option>
            <option value="compress">{t('tools.xml.actions.compress')}</option>
          </select>
        </label>
        <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
          {t('tools.xml.indent')}
          ...
          >
            <option value={2}>{t('tools.xml.indent2')}</option>
            <option value={4}>{t('tools.xml.indent4')}</option>
          </select>
        </label>
      ...
      {result && !result.ok && (
        <p role="alert" className="text-sm text-red-600 dark:text-red-400">
          {translateToolError('tools.xml', result)}
        </p>
      )}
    </div>
  );
