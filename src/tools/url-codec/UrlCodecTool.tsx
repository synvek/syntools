import { useMemo, useState } from 'react';
import { useTranslation } from 'react-i18next';
import { IOTextArea } from '@/core/components/IOTextArea';
import { CopyButton } from '@/core/components/CopyButton';
import { ClearButton, OptionBar, SwapButton } from '@/core/components/ActionButtons';
import { ShareButton } from '@/core/components/ShareButton';
import { translateToolError } from '@/core/i18n/helpers';
import { readSharedState } from '@/core/lib/share';
import { urlDecode, urlEncode, type UrlMode } from './core';

type Direction = 'encode' | 'decode';

export default function UrlCodecTool() {
  const { t } = useTranslation();
  const init = useMemo(() => readSharedState({ i: '', d: 'encode', m: 'component' }), []);
  const [input, setInput] = useState(init.i);
  const [direction, setDirection] = useState<Direction>(init.d === 'decode' ? 'decode' : 'encode');
  const [mode, setMode] = useState<UrlMode>(init.m === 'full' ? 'full' : 'component');

  const result = useMemo(
    () => (direction === 'encode' ? urlEncode(input, mode) : urlDecode(input, mode)),
    [input, direction, mode],
  );
  const output = result.ok ? result.value : '';

  const swap = () => {
    if (!result.ok) return;
    setInput(result.value);
    setDirection(direction === 'encode' ? 'decode' : 'encode');
  };

  return (
    <div className="flex flex-col gap-4">
      <OptionBar>
        <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
          {t('common.operation')}
          <select
            value={direction}
            onChange={(e) => setDirection(e.target.value as Direction)}
            className="rounded-md border border-gray-300 bg-white px-2 py-1 text-sm focus:border-blue-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900"
          >
            <option value="encode">{t('common.encode')}</option>
            <option value="decode">{t('common.decode')}</option>
          </select>
        </label>
        <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
          {t('tools.url.mode')}
          <select
            value={mode}
            onChange={(e) => setMode(e.target.value as UrlMode)}
            className="rounded-md border border-gray-300 bg-white px-2 py-1 text-sm focus:border-blue-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900"
          >
            <option value="component">{t('tools.url.modes.component')}</option>
            <option value="full">{t('tools.url.modes.full')}</option>
          </select>
        </label>
        <SwapButton onSwap={swap} disabled={!result.ok} />
        <ShareButton getState={() => ({ i: input, d: direction, m: mode })} />
      </OptionBar>

      <div className="flex flex-col gap-4 lg:flex-row">
        <IOTextArea
          label={
            direction === 'encode'
              ? t('tools.url.labels.rawText')
              : t('tools.url.labels.encodedText')
          }
          value={input}
          onChange={setInput}
          placeholder={
            direction === 'encode'
              ? t('tools.url.placeholders.encode')
              : t('tools.url.placeholders.decode')
          }
          actions={<ClearButton onClick={() => setInput('')} disabled={!input} />}
        />
        <IOTextArea
          label={t('common.result')}
          value={output}
          readOnly
          actions={<CopyButton text={output} disabled={!output} />}
        />
      </div>

      {!result.ok && input && (
        <p role="alert" className="text-sm text-red-600 dark:text-red-400">
          {translateToolError('tools.url', result)}
        </p>
      )}
    </div>
  );
}
